...
class Save(abc.ABC):
    ...
    def output(self, output: str = None, clouds: CloudManager = None,
               run_command: bool = True, ffmpeg_bin: str = 'ffmpeg', monitor: callable = None, monitor_args=None, **options):
        """
        @TODO: add documentation
        """
        ...
        if run_command:
            self.run(ffmpeg_bin, monitor, monitor_args, **options)
    ...
    def _run(self, ffmpeg_bin, monitor: callable = None, monitor_args=None, **options):
        """
        @TODO: add documentation
        """
        with Process(self, command_builder(ffmpeg_bin, self), monitor, monitor_args, **options) as process:
            self.pipe, err = process.run()

    async def async_run(self, ffmpeg_bin, monitor: callable = None, monitor_args=None, **options):
        """
        @TODO: add documentation
        """
        self._run(ffmpeg_bin, monitor, monitor_args, **options)

    def run(self, ffmpeg_bin, monitor: callable = None, monitor_args=None, **options):
        """
        @TODO: add documentation
        """
        if async_run := options.pop('async_run', True):
            asyncio.run(self.async_run(ffmpeg_bin, monitor, monitor_args, **options))
        else:
            self._run(ffmpeg_bin, monitor, monitor_args, **options)
