...
class S3(Clouds):
    ...
    def upload_directory(self, directory, **options):
        ...
        try:
            for file in files:
                self.s3.upload_file(join(directory, file), bucket_name, join(folder, file).replace("\\", "/"), Config=TransferConfig(use_threads=False))
    ...
    def download(self, filename=None, **options):
        ...
        try:
            with filename as f:
                self.s3.download_fileobj(bucket_name, key, f, Config=TransferConfig(use_threads=False))
